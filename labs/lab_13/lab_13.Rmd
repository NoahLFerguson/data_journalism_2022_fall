...
```{r}
ag_url <- "https://electionresults.utah.gov/elections/countyCount/451556070"
```
...
```{r}
download.file("https://elections.utah.gov/Media/Default/2018%20Election/2018%20General%20Election%20Canvass.xlsx", "utah.pdf")
```

```{r}
utah2018 <- read_excel("2018 General Election Canvass.xlsx", sheet=2, skip = 2) %>% 
...
```{r}
joined_utah <- utah2018 %>% 
inner_join(results, by= ("county"))
```
...
**A1.** Evan McMullin outperformed Jenny Wilson in all but one Utah county when comparing the 2018 and 2022 Utah general election results. The disparity was particularly drastic in large counties like Salt Lake City, where McMullin garnered over 31,000 more votes than Wilson. McMullin ran as an independent in 2018, while Wilson won the mayor position as a Democrat in 2022.

```{r}
mcmullin_wilson <- joined_utah %>% 
  select(county, wilson, mcmullin) %>% 
  mutate(
    difference = (wilson-mcmullin))
```
...
**A2** Mitt Romney outperformed Mike Lee in large counties like Salt Lake and Davis by a good margin, while Lee picked up more votes in smaller counties. Lee won, percentage-wise, in small counties like Piute and Sanpete. Romney is a much more well-known name given his presidential run against future president Barack Obama back in 2012, but Lee still put up numbers during his Utah run.

```{r}
lee_romney <- joined_utah %>% 
  select(county, lee, romney) %>% 
  mutate(
    difference = (lee-romney))
```
...
**A3** i couldn't beat the buzzer on this one :(
