...
md_gov_county_cox <- md_gov_county %>% 
  mutate(
    total= (cox + moore + lashar + wallace + harding + sefcik + write_ins)) %>% 
  mutate(
    cox_percent= (cox / total)*100)
```

```{r}
md_gov_county_hogan <- md_gov_county_18 %>% 
  mutate(
...
```{r}
joined_gov_county <- md_gov_county_cox %>% left_join(md_gov_county_hogan, by="geoid")
```

```{r}
joined_gov_county_final <- joined_gov_county %>% 
  mutate(
    percent_difference= (hogan_percent - cox_percent))
```

```{r}
write_csv(joined_gov_county_final, "data/joined_gov_county_final.csv")
```

Question 1 Bar Graph: https://datawrapper.dwcdn.net/53QTu/1/
...
**A2.** In-person voting nearly returned to pre-pandemic turnout in 2022 election after most people remained home in 2020, while the number of Early Votes remained constant throughout the three election cycles.
...
Question 2 Line Graph: https://datawrapper.dwcdn.net/dLqee/1/
...
```{r}
md_final_total <- md_total %>% 
  mutate(
    percent_total= (total / overall_total)
  )
```

```{r}
write_csv(md_final_total, "data/md_final_total.csv")
```

Question 3 Map: https://datawrapper.dwcdn.net/QLmlB/1/

This map is not what I wanted to have submitted, I will ask questions in class on Monday!
