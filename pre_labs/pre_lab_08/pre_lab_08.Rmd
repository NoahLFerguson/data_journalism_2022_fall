...
**Answer** maryland_top_counties is a subset of md_voters. The arrange function puts the counties wit the biggest difference in registered voters over a two year span at the top and the smallest at the bottom. The select and head functions grabs the top ten counties to whittle down the data frame into the ten counties with the greatest change in registered voters from 2020-2022.
...
**Answer** I see an empty gray box/container
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see a messy bar graph with counties in alphabetical order on the x axis
...
**Answer** The counties are now ordered by count from least to greatest instead of alphabetized.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The axis have switched, so the counties are not on the y axis and the bars move sideways instead of upwards.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The graph now doesn't have the gray background box.
...
**Answer** The graph's background box is now a different color (i'm colorblind so im not sure the exact color lol)
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The graph now has a title and the axis have different titles that are more representative of the data.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I'm looking at total daily winred contributions from the beginning of April to the end of June.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a line graph depicting daily contributions from April to July. The graph doesn't have individual days on the x axis, however.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The x axis now has specific dates spread out by seven days each (April 4, April 11, April 18).
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The x axis dates are now angled for better readability.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
