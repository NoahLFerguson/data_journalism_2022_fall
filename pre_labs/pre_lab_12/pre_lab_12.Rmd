...
**Answer** I got the information from the first page, but the data is cluttered and needs to be cleaned up before I can use it.
...
**Answer** This code cleans up the dataframe a lot, cutting the first two rows and renaming the columns while excluding unwanted commas.
...
**Task** Run the following code and describe the result.
**Answer** Renamed the columns, removed commas and foxed datatypes, as well as excluding rows with "NAs" and adding a tenth month and the year 2022.
...
**Answer** Democrats have higher rates of new registrations than Republicans in every category, and so do unaffiliated registrants, in most cases. Curious as to why that is.
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** The majority of the removals are by county transfer. Is that always the case, or is this an unusual outlier?
```{r}
removals <- removals %>% 
  mutate(pct_dem = DEM/total *100, pct_rep = REP/total *100, pct_una = UNA/total *100)
...
**Answer** 16 rows on the new dataframe compared to 17 on the June dataframe
...
**Answer** They have both positive and negative numbers for each (democrat, republican, unaffiliated)
...
**Answer** This chart really is't showing much except for that Democrats are pretty consistent in a multitude of ways of voter registration.
...
**Answer** ERIC Report is much higher in October than June.
