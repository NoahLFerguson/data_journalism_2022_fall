...
**Answer** The code is mutating to show how many votes were cast on election day for each candidate next to the total number of votes, as well as the percentage of election day votes cast per candidate.
...
**Answer** The answer is changed because the code formatted the graph in descending order to see which candidates had the highest percentage of election day votes out of their total number of votes.
...
**Answer** I think both answers are interesting when you juxtapose them, because it shows which counties are more likely to vote on election day versus not on election day. For instance, Talbot county looks to be a frequent non-election day county, while Allegany is the opposite and mostly votes in person.
...
**Answer** str_to_lower returns string with all alphabetic characters converted to lowercase, while str_to_title() fixes capitalization
...
**Answer** Filtering by highest to lowest amount donated in a single donation
...
**Answer** The "less than $100" category
...
**Answer** The biggest difference I see from the two datasets is that the second is much cleaner with less columns. The data no longer shows the breakdown of who voted when/where, and now just shows the total amount of votes per candidate. It's a much simpler data set now, but easier to read/discern information from.
...
**Answer:** I got an error when running the above code, but I assume the code works to fix the character date by converting it into an actual date, which is far more efficient than the code thinking the dates are character columns.
...
**Answer** I received an error when trying to run the last bit of code. I tried installing "lubridate" but I believe it's already installed.
