...
**Answer** 1640 rows have problems.mThe problem that R is describing is that the code is expecting to see TRUE or FALSE on the provisional and limited columns. The rows with errors don't have data in those cells.
...
**Answer** I don't see any parsing error messages when I run the data 
...
**Answer** the district field's data type is <dbl> and the precinct field's data type is <chr>. The mail column's data type is <dbl>.
```{r}
...
**Answer**: The count is 402,345 for NA. The implication there is that those 402,345 votes weren't cast by mail.
...
**Answer**The 12 counts from 1900-01-01 are clearly not legitimate since the people would be well over 100 years old. That date must be a starting point for registration months, which unfortunately would skew the data.
```{r}
yadkin_voters %>% 
...
**Answer** One of the groups I'm interested in is the one person whose reason for not being an active voter was because of "unavailable essential information." I explored by filtering that group category to see the one name, it's a 55-year old woman named Denise. We might not be able to find out what the information she excluded was in the code, but I just thought it was interesting since she was literally the only one that had unavailable essential information.
...
```{r}
yadkin_voters %>%
  filter(voter_status_reason_desc == "UNAVAILABLE ESSENTIAL INFORMATION")
```
...
**Answer** There are 14 rows in the demonstration set
...
**Answer** <chr> is the data type for the amount field
```{r}
glimpse(conowingo)
```
...
**Answer**Changing the 1_linenumber column to x1_linenumber makes it so that the data type makes sense. the number 1 in the column implies that the colums data type needs to be a <dbl> since there's a stray number. By adding the x, the <chr> data type makes sense.
...
**Answer** The error says:
Error in `summarise()`:
! Problem while computing `total_amount = sum(amount)`.
Caused by error in `sum()`:
! invalid 'type' (character) of argument

It says invalid type (character), so I'm assuming that the code can't run because it's a character data type and not a <dbl> data type.
...
**Answer** The amount is now a <dbl> datatype and it now works, which explains the error above.
...
**Answer** There is one duplicate. The donor's name is Derrick (the uncool spelling) Hamilton.
...
**Answer** 13 rows in new data frame
...
**Answer** The code mutated the zip code parameters so that the new code omitted the last four numbers of the nine-digit zip codes to make all of the zip codes five digits.
...
**Answer** 1 misspelling (Conowingoo)
